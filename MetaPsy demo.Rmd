...
ggplot(mp_phq9_all, aes(d_local, d_norm)) +
  ...
  geom_errorbarh(aes(xmin = d_local - sqrt(vi_local), 
                     xmax = d_local + sqrt(vi_local)), 
                alpha = 0.5, height = 0) +
  geom_errorbar(aes(ymin = d_norm - sqrt(vi_norm), 
                    ymax = d_norm + sqrt(vi_norm)), 
               alpha = 0.5, width = 0) +
  geom_point(size = 2) +
  coord_fixed() +
